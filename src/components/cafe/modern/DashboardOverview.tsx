...
export function DashboardOverview({ finance, periodString }: DashboardOverviewProps) {
  ...
  const StatCard = ({ title, value, subValue, icon: Icon, color, delay }: any) => (
    <motion.div
      ...
      viewport={{ once: true }}
      transition={{ delay }}
      className="premium-card p-6 flex flex-col justify-between group h-full"
    >
      ...
      <div>
        ...
        <h3 className="fintech-kpi text-2xl text-gray-900">{value}</h3>
        {subValue && (
          <p className="text-[10px] font-bold text-gray-400 mt-2 line-clamp-1">{subValue}</p>
        )}
      </div>
    </motion.div>
  );
  ...
  return (
    <div className="space-y-8">
      ...
      <motion.div 
        ...
        className={`rounded-[3rem] p-8 sm:p-12 text-white relative overflow-hidden shadow-premium group
          ${kpi.soldeNet >= 0 
      ...
      >
        ...
        <div className="relative z-10">
          <div className="inline-flex items-center gap-2 px-4 py-2 bg-white/10 backdrop-blur-md rounded-full border border-white/10 text-[10px] font-black uppercase tracking-[0.2em] mb-8">
             <Activity size={12} className={kpi.soldeNet >= 0 ? "text-emerald-400" : "text-red-400 animate-pulse"} />
             Performance Financière • {periodString}
          </div>
          
          <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-12">
            <div>
              <p className="text-white/60 font-medium text-lg mb-2">Solde Net Restant</p>
              <div className="flex items-baseline gap-4">
                <h1 className="fintech-kpi text-6xl sm:text-8xl">
                  {kpi.soldeNet > 0 ? '+' : ''}{formatPrice(kpi.soldeNet)}
                </h1>
                <span className="text-3xl sm:text-4xl font-black text-white/30 uppercase">F CFA</span>
              </div>
            </div>

            <div className="flex flex-wrap items-center gap-4">
              <div className={`px-6 py-4 rounded-3xl backdrop-blur-xl border border-white/10 flex items-center gap-4 ${kpi.soldeNet >= 0 ? 'bg-emerald-500/20' : 'bg-red-500/20'}`}>
                <div className={`w-10 h-10 rounded-full flex items-center justify-center ${kpi.soldeNet >= 0 ? 'bg-emerald-400 text-dmn-green-950' : 'bg-red-400 text-black'}`}>
                   {kpi.soldeNet >= 0 ? <TrendingUp size={20} strokeWidth={3} /> : <TrendingDown size={20} strokeWidth={3} />}
                </div>
                <div>
                   <p className="text-[10px] font-black text-white/50 uppercase tracking-wider">Marge Nette</p>
                   <p className="text-2xl font-black">{kpi.netMargin.toFixed(1)}%</p>
                </div>
              </div>
              
              <div className="px-6 py-4 rounded-3xl backdrop-blur-xl border border-white/10 bg-white/5">
                <p className="text-[10px] font-black text-white/50 uppercase tracking-wider">Rentabilité Brute</p>
                <div className="flex items-center gap-2">
                   <p className="text-2xl font-black text-dmn-gold">{kpi.margin.toFixed(1)}%</p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-10 pt-12 mt-12 border-t border-white/10">
            <div className="group/item cursor-default">
              <div className="flex items-center gap-2 mb-2">
                ...
                <p className="text-white/40 text-[10px] font-black uppercase tracking-widest">Recherche de Profits</p>
              </div>
              <p className="text-white/60 text-xs font-bold mb-1">Chiffre d'Affaires</p>
              <p className="text-3xl font-black group-hover/item:text-white transition-colors">{formatPrice(sales.total)} <span className="text-sm opacity-40">F</span></p>
            </div>
            
            <div className="group/item cursor-default">
              <div className="flex items-center gap-2 mb-2">
                <div className="w-1.5 h-1.5 rounded-full bg-red-400 group-hover/item:scale-150 transition-transform" />
                <p className="text-white/40 text-[10px] font-black uppercase tracking-widest">Investissement Direct</p>
              </div>
              <p className="text-white/60 text-xs font-bold mb-1">Production (Brute)</p>
              <p className="text-3xl font-black text-red-200/80 group-hover/item:text-red-100 transition-colors">-{formatPrice(costs.totalProd)} <span className="text-sm opacity-40">F</span></p>
            </div>

            <div className="group/item cursor-default">
              <div className="flex items-center gap-2 mb-2">
                <div className="w-1.5 h-1.5 rounded-full bg-orange-400 group-hover/item:scale-150 transition-transform" />
                <p className="text-white/40 text-[10px] font-black uppercase tracking-widest">Charges Externes</p>
              </div>
              <p className="text-white/60 text-xs font-bold mb-1">Fonctionnement</p>
              <p className="text-3xl font-black text-orange-200/80 group-hover/item:text-orange-100 transition-colors">-{formatPrice(costs.operating)} <span className="text-sm opacity-40">F</span></p>
            </div>
          </div>
        </div>
      </motion.div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        <StatCard 
          ...
          subValue="Bénéfice Direct (Ventes - Coûts Directs)"
          icon={Activity} 
          color="blue"
          delay={0.1}
        />
        ...
        <StatCard 
          ...
          subValue="Estimation sur mouvement mensuel"
          icon={PieChartIcon} 
          color="purple"
          delay={0.4}
        />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        <motion.div 
          ...
          className="lg:col-span-5 premium-card p-10 flex flex-col relative overflow-hidden"
        >
          ...
          <div className="flex items-center gap-3 mb-10 relative z-10">
            <div className="w-10 h-10 rounded-full bg-dmn-green-900 text-white flex items-center justify-center shadow-lg shadow-dmn-green-900/20">
              <Lightbulb size={20} />
            </div>
            <h3 className="text-xl font-black text-gray-900 tracking-tight">Intelligence Commerciale</h3>
          </div>

          <div className="space-y-8 relative z-10 flex-1">
            <div className="p-6 rounded-[2rem] bg-gray-50 border border-gray-100 group hover:bg-white hover:shadow-soft transition-all duration-300">
               <div className={`p-2 rounded-lg text-[10px] font-black uppercase tracking-widest w-fit mb-3 ${
            ...
            <div className={`p-8 rounded-[2rem] border-2 border-dashed ${
               ...
               <p className="text-xs font-black text-gray-400 uppercase tracking-[0.2em] mb-4">Plan d'Optimisation</p>
               <p className="text-gray-900 font-bold">
                 ...
                 {insightTone === 'critical' && "Déséquilibre identifié. Reconsidérer les remises revendeurs ou ajuster le prix public (cible recommandée : 6000F/kg)."}
        ...
        <motion.div 
          ...
          className="lg:col-span-7 premium-card p-10"
        >
          <div className="flex items-center justify-between mb-10">
            <h3 className="text-xl font-black text-gray-900 tracking-tight">Répartition des Flux</h3>
            <div className="flex gap-2">
               <div className="flex items-center gap-1.5">
                  <div className="w-2 h-2 rounded-full bg-emerald-500" />
                  <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Revenus</span>
               </div>
               <div className="flex items-center gap-1.5 ml-4">
                  <div className="w-2 h-2 rounded-full bg-dmn-coffee" />
                  <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Dépenses</span>
               </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-10 h-[350px]">
             {/* Pie Chart */}
             <div className="flex flex-col h-full bg-gray-50/50 rounded-3xl p-6 border border-gray-100">
                <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-6">Origine des Coûts</p>
                {costsData.length > 0 ? (
                  ...
                  <div className="flex-1 flex items-center justify-center text-gray-300 font-black text-[10px] uppercase tracking-widest">Données insuffisantes</div>
                )}
             </div>

             {/* Bar Chart */}
             <div className="flex flex-col h-full bg-gray-50/50 rounded-3xl p-6 border border-gray-100">
                <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-6">Ratio Ventes / Charges</p>
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={salesVsCostsData} margin={{ top: 20, right: 0, left: 0, bottom: 0 }}>
                    ...
                    <Tooltip 
                       ...
                       formatter={(value) => `${formatPrice(value as number)} F`} 
                    />
                    <Bar dataKey="Ventes" fill="#10B981" radius={[12, 12, 12, 12]} maxBarSize={30} />
                    <Bar dataKey="Coûts" fill="#6B3F2A" radius={[12, 12, 12, 12]} maxBarSize={30} />
                  </BarChart>
                </ResponsiveContainer>
             </div>
          </div>
